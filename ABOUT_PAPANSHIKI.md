...
- **Google Sheets (CSV公開)**: リアル株価データ、およびぱぱん銀行のUSD/JPY現在レート・直近30日履歴の取得
...
| `math_molkky.html` | ひきざんモルック | 算数・戦略ゲーム | A（スコアの10分の1＝0〜125pt） | 6〜12歳 |
...
| `crossword/index.html` | もりのクロスワード | パズル | D (最大60pt) | 6〜8歳 |
...
| `molkky.html` | 森のモルック | 戦略ゲーム | D (15/30/60pt) | 6〜12歳 |
...
- 現在レートはGoogle Sheetsの公開CSVにある`rate_source`シートから取得し、四捨五入した整数を交換に使用する
- 30日間チャートは公開CSVの`rate_public`シートから取得した小数値を使用し、ブラウザの`localStorage`には保存しない
- 現在レートを取得できない場合は仮レート150で交換を継続し、履歴を取得できない場合はチャート内に読み込み失敗を表示する
